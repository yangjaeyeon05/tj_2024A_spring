...
public class Step2 {
    public static void main(String[] args) {
        ...
        // 활용 : Scanner 클래스에는 문자 입력 메소드가 없다.
        ...
        // [4] 문자열.subString(시작인덱스 , [끝인덱스]); : 문자열을 인덱스 기준으로 잘라낸 문자열 반환
            // ssn = 012312 - 1230123;
        ...
        System.out.println("ssn.split(\"-\") = " + ssn.split("-")); // "-" 기준으로 문자열 분해
        ...
        boolean findCheck = subject.contains("파이썬");
        ...
        // 문자와 바이트 관계
            // 영문 / 특정 특수문자 : 문자 1개당 1바이트
            // 한글 : 문자 1개당 2바이트 + 규칙(UTF-8-인코딩헤더규칙)1바이트 = > 총 3바이트
        byte _byte1 = 'a';
        System.out.println("_byte1 = " + _byte1);   // 97   // 유니코드(아스키코드기반)
        _byte1++;
        ...
        System.out.println("가".getBytes().length);  // 3 : 한글
            // - char : 문자 1개 표현 , 65536글자표현 , 부호 : signed 부호없음 : unsigned   vs  short : +- 3만
        char _char1 = '가';
        ...
        // [활용] 난수 문자열을 만들기 , 임시비밀번호 , 인증번호
        System.out.println(new Random().nextInt()); // int타입의 허용번위 내 난수 생성
        System.out.println(new Random().nextInt(26));   // 0 ~ 25 사이의 난수 생성
        System.out.println(new Random().nextInt(26)+97);   // 97 ~ 122 사이의 난수 생성
        System.out.println((char)(new Random().nextInt(26)+97));   // a ~ z 사이의 영소문자 랜덤 생성
        String newPwd = "";
        for(int i = 0; i < 10; i++){
