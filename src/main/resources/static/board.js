...
function boardCreate(){ 
  ...
  $.ajax({
    ...
    url : "/board/create" , // HTTP 통신할 경로URL , 컨트롤러 매핑
  ...
  $.ajax({
    ...
    success : function(response){
        ...
        let html = `<h3> 상세 페이지 </h3>
                    ...
                    <button onclick="boardUpdate( ${ bno } )">수정</button>
                    <button onclick="boardDelete( ${ bno } )">삭제</button>`;
        // 3. 출력/대입 
...
function boardUpdate(bno){
  ...
  $.ajax({
      method : 'put' ,                // HTTP 메소드
      url : `/board/update` , // HTTP 통신할 경로URL , 컨트롤러 매핑
      data : { "bno" : bno , "bpwd" : bpwd , "bcontent" : bcontent} , 
      success : function(response){    // HTTP 성공응답 , 컨트롤러가 return한 값
...
function boardDelete(bno){
  ...
  $.ajax({
      ...
      url : `/board/delete` , // HTTP 통신할 경로URL , 컨트롤러 매핑
