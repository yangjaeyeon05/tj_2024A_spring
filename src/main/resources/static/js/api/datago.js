...
function api1(){
    $.ajax({
        async : false , 
        mathod : "get" , 
        url : "https://api.odcloud.kr/api/15102672/v1/uddi:5e2a4b30-28fb-4e8f-bc44-9a6db8a6a8db?page=1&perPage=39&serviceKey=xWhYT%2FzJWCqE1zXpxSxKi%2FhyvsA99UQM8oXscf9m9lblUO5rGFvAkPQlvRKcD7Pm7rCEHaZWN0oGDeEaaT9bjQ%3D%3D" , 
        success : (r)=>{
            console.log(r);
            /*
                r : 다양한 속성을 가진 응답 객체
           ...
           dataArray.forEach(data => {
            // 보통 . 으로 불러오지만 key 값에 특수문자가 포함될 수 있으므로 []로 불러오기
                html += `<tr>
                            <td> ${data['상호']} </td>     
                            <td> ${data['전화번호']} </td>
                            <td> ${data['주소']} </td>
...
function api2(){
    $.ajax({
        async : false , 
        method : "get" , 
        url : "https://api.odcloud.kr/api/15051492/v1/uddi:852bbc11-63ed-493e-ab09-caaaf54fd144?page=1&perPage=35&serviceKey=xWhYT%2FzJWCqE1zXpxSxKi%2FhyvsA99UQM8oXscf9m9lblUO5rGFvAkPQlvRKcD7Pm7rCEHaZWN0oGDeEaaT9bjQ%3D%3D" , 
        success : (r)=>{
            console.log(r);
            let dataArray = r.data;
            console.log(dataArray);
...
/*
    ...
    2. 로그인
    3. 플랫폼 신청
        1. 상단메뉴 -> [내 어플리케이션] -> [어플리케이션 추가] 
    4. 어플리케이션 선택
        사이드바 메뉴
        [앱키] : 카카오API 사용할 때 사용되는 인증 키
            JavaScript 키 확인 , 카카오 지도 활용
            <script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=133a0498dd3d00398be487d020a4b281"></script>
        [플랫폼] : 카카오API 사용할 때 web URL 등록
            Web 플랫폼 등록
                http://localhost:8080
                http://192.168.30.28:8080
*/
function api3(data){    // 매개변수 : api2에서 전달받은 약국정보리스트
    ...
    // 4. 마커를 표시할 위치와 title 객체 배열입니다 
        ...
        // data는 api2 함수에서 공공데이터포털의 인천동구약국현황 정보를 매개변수로 전달 받은 객체              
